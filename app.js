...
async function loadLogData() {
    ...
    sheetsData.log = rows.map(row => ({
        ...
        VO2Max: row[4] === 'TRUE',
        Walk: row[5] === 'TRUE',
        SportDay: row[6] && row[6] !== 'FALSE' ? row[6] : false,
        Sauna: row[7] === 'TRUE',
...
async function saveDayData(date, activities) {
    ...
    const rowData = [
        ...
        activities.SportDay ? activities.SportDay : 'FALSE',
...
function renderWeekView() {
    ...
    days.forEach((date, index) => {
        ...
        if (activeActivities.length > 0) {
            ...
            blocksHTML = activeActivities.map(a => {
                const label = (a.id === 'SportDay' && typeof dayData[a.column] === 'string')
                    ? dayData[a.column] : a.short;
                return `<div class="day-strip-block${sizeClass}" style="background:${a.color}">${label}</div>`;
            }).join('');
    ...
    summary.innerHTML = ACTIVITIES.filter(a => a.id !== 'RestDay').map(a => {
        ...
        const belowGoal = goal > 0 && count < goal;
        const isZero = count === 0 && goal === 0;
        return `
            <div class="summary-counter">
                <div class="summary-counter-num ${isZero ? 'zero' : ''} ${goalMet ? 'goal-met' : ''} ${belowGoal ? 'below-goal' : ''}">${count}</div>
                <div class="summary-counter-label ${goalMet ? 'goal-met' : ''} ${belowGoal ? 'below-goal' : ''}">${a.short}</div>
            </div>
        `;
    }).join('');
}
...
function openActivityModal(date) {
    ...
    ACTIVITIES.forEach(activity => {
        ...
        const displayName = (activity.id === 'SportDay' && typeof isSelected === 'string')
            ? `${activity.name}: ${isSelected}` : activity.name;
        btn.innerHTML = `
            ${activity.icon ? activity.icon + ' ' : ''}${displayName}
            ${isSelected ? '<span class="modal-check">✓</span>' : ''}
        `;
        btn.addEventListener('click', () => {
            if (activity.id === 'SportDay') {
                openSportSubSelector();
            } else {
                modalToggleActivity(activity.id);
            }
        });
...
function modalToggleActivity(activityId, sportValue) {
    ...
    let newValue;
    if (activityId === 'SportDay') {
        // Tap same sport again → deselect; otherwise set the new sport
        newValue = (dayData.SportDay === sportValue) ? false : (sportValue || false);
    } else {
        newValue = !dayData[activity.column];
    }
...
const SPORT_OPTIONS = ['Ski', 'Rando', 'MTB', 'Hike', 'Swim'];

function openSportSubSelector() {
    if (!modalDate || !isSignedIn) return;

    // Toggle: close if already open
    const existing = document.getElementById('sportSubSelector');
    if (existing) { existing.remove(); return; }

    const dateStr = formatDateForSheets(modalDate);
    const dayData = getDayData(dateStr);
    const currentSport = typeof dayData.SportDay === 'string' ? dayData.SportDay : null;
    const sportActivity = ACTIVITIES.find(a => a.id === 'SportDay');

    const panel = document.createElement('div');
    panel.id = 'sportSubSelector';
    panel.className = 'sport-sub-selector';
    panel.innerHTML = `
        <div class="sport-sub-header">
            <span>Choose sport</span>
            <button class="sport-sub-close" id="sportSubCloseBtn">✕</button>
        </div>
        <div class="sport-sub-grid">
            ${SPORT_OPTIONS.map(sport => `
                <button class="sport-sub-btn${currentSport === sport ? ' selected' : ''}"
                    style="${currentSport === sport ? `background:${sportActivity.color};border-color:rgba(255,255,255,0.2)` : ''}"
                    data-sport="${sport}">${sport}</button>
            `).join('')}
            ${currentSport ? '<button class="sport-sub-btn deselect" data-sport="">✕ Remove</button>' : ''}
        </div>
    `;

    document.getElementById('modalGrid').after(panel);

    panel.querySelectorAll('.sport-sub-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            const sport = btn.dataset.sport;
            panel.remove();
            modalToggleActivity('SportDay', sport || false);
        });
    });

    document.getElementById('sportSubCloseBtn').addEventListener('click', () => panel.remove());
}
...
function updateModalSelected(dateStr) {
    ...
    if (selected.length > 0) {
        container.innerHTML = selected.map(a => {
            const label = (a.id === 'SportDay' && typeof dayData[a.column] === 'string')
                ? dayData[a.column] : a.short;
            return `<span class="modal-chip" style="background:${a.color}">${label}</span>`;
        }).join('');
    } else {
